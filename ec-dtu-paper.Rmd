...
f3c <- plot_bottomly_boxplot(results, cols, title='Recall fraction (intersect)', toplot='TPR')
...
s3b <- plot_bottomly_boxplot(results, cols, title='Recall fraction (union)', toplot='TPR', lines=T)
...
s3d <- plot_bottomly_boxplot(results, cols, title='Recall fraction (intersect)', toplot='TPR', lines=T)
...
s3f <- plot_bottomly_boxplot(results, cols, title='Recall fraction (individual)', toplot='TPR', lines=T)
...
Kallisto[4] versus salmon's[5] performance on the Bottomly subset testing experiments, using each method's significant genes from the full (10 vs. 11) run as the truth set for calculating both metrics.
...
Shows the performance of the equivalence class, transcript (Salmon) and exon-based (DEXSeq-count) methods with _DRIMSeq_ filtering (minimum feature expression = 10, minimum gene expression = 10, gene must be expressed in at all 6 samples and transcript must be expressed in at least 3 samples).
...
cols <- c('transcripts_salmon' = '#d73027',
          'transcripts_salmon_filt' = '#fc8d59',
          'equivalence_class' = '#fdae61',
          'equivalence_class_filt' = '#fee090',
          'dexseq_count_exons' = '#91bfdb',
          'dexseq_count_exons_filt' = '#4575b4')
...
An example EC usage plot for a single gene from the hsapiens DECU results. Log EC usage is shown across each equivalence class for both conditions. Yellow blocks indicate significant ECs. As ECs do not correspond easily to genomic locations, no special ordering is applied the the ECs. In the given example, transcripts mapping to each significant EC can be observed in the data. The significant ec142580 corresponds to a single transcript (ENST00000443443), indicating that interpretation can be straight-forward if the EC is associated with a single transcript.

```{r SupplementaryFigure7, fig.width=12, fig.height=6}
# create EC/gene_id lookup table
lookup <- distinct(hs_ec[,c('gene_id', 'ensembl_id', 'symbol', 'ec_names')])
lookup$id <- paste(lookup$gene_id, lookup$ec_names, sep=':')
colnames(lookup)[2] <- 'transcript'

plot_ec_usage(hs_ec_results$dexseq_results, 'ENSG00000004766', lookup)
```
...
group <- rep(c('c1','c2'), each=12)
lv_ec <- run_diffsplice(lv_ec, group, 'sample', feature='ec')
lv_tx <- run_diffsplice(lv_tx, group, 'sample', feature='tx')
lv_ex <- run_diffsplice(lv_ex, group, 'lv', feature='ex')
```

# Supplementary Figure 8

Performance of exon count DTU derived from DEXSeq's count pipeline and EC and transcript DTU derived from salmon on data from Love et al.[6] for nominal FDR cutoffs of 0.01, 0.05 and 0.1.
...
txinfo$ds_status <- txinfo$gene %in% dtu_genes

truth <- data.frame(gene=lookup$gene, ds_status=as.numeric(lookup$gene %in% dtu_genes))
...
[6] Love, M. I., Soneson, C., & Patro, R. (2018). Swimming downstream: statistical analysis of differential transcript usage following Salmon quantification. F1000Research, 7, 952. https://doi.org/10.12688/f1000research.15398.1